...
void Attribute::setKey(const QString &k, const AttributeType &t)
{
    ...
    qWarning() << QString(
                      "Attribute::setKey (%1)  attribute already defined!\n")
                      .arg(k)
                      .toUtf8();
}
...
void Attribute::setValue(const QString &v)
{
    if (!table) {
        qWarning()
            << QString("Attribute::setValue (%1)  No table defined!").arg(v);
        return;
    }
    if (!definition) {
        qWarning() << QString("Attribute::setValue (%1)  No attribute defined!")
                          .arg(v);
        return;
    }
...
void AttributeDef::setType(const AttributeType &t) { type = t; }

AttributeType AttributeDef::getType() { return type; }
...
void AttributeDef::setKey(const QString &k) { key = k; }
...
QVariant AttributeDef::getValue() { return QVariant(); }

QString AttributeDef::getKey() { return key; }
...
AttributeTable::~AttributeTable() { clear(); }

void AttributeTable::clear() { attdefs.clear(); }
...
QStringList AttributeTable::getTypes() { return typeList; }
