                                   ...
                                   const QXmlAttributes &atts)
{
    ...
    */
    ...
    else if (eName == "slide" && state == StateMap) { // XML-FIXME-1 port and test
        state = StateMapSlide;
    ...
    else if (eName == "vymnote" && // XML-FIXME-1 test
             (state == StateMapCenter || state == StateBranch ||
              state == StateInit)) {
        if (state == StateInit)
        // Only read some stuff like VymNote or Heading
    ...
    else if (eName == "html" && // XML-FIXME-1 port legacy and test
             (state == StateHtmlNote ||
    ...
    else if (state == StateHtml) { // XML-FIXME-1 port legacy and test
                                 ...
                                 const QString &eName)
{
    ...
    */
    ...
    case StateBranch:
        ...
        // XML-FIXME OBSOLETE, breaks build lastBranch->getBranchContainer()->updateStyles(BranchContainer::RelinkBranch);
        model->emitDataChanged(lastBranch);
        lastBranch = (BranchItem *)(lastBranch->parent());
    ...
    case StateTask:
        break;
    case StateHeading:  // XML-FIXME-1 test with legacy
    ...
    case StateMapSetting:
        ...
        break;
    case StateVymNote:  // XML-FIXME-1 test
...
bool parseVYMHandler::characters(const QString &ch) // XML-FIXME-0 check compatibility
{
    ...
    case StateHtmlNote: // Only for compatibility
        htmldata = ch;
        break;
    case StateHtml: // XML-FIXME-2 test legacy
        htmldata += ch_org;
        break;
    case StateHeading:  // XML-FIXME-1 test legacy
        htmldata += ch;
        break;
    default:
        return false;
