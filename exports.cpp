...
void ExportAO::doExport()   
{
    ...
    {
        QMessageBox::critical (0, tr("Critical Export Error"), tr("Could not export as AO to %1").arg(filePath));
...
void ExportASCII::doExport()
{
    ...
    if ( !file.open( QIODevice::WriteOnly ) )
    {
        QMessageBox::critical (0, tr("Critical Export Error"), tr("Could not export as ASCII to %1").arg(filePath));
        mainWindow->statusMessage(QString(QObject::tr("Export failed.")));
        return;
    }
    QTextStream ts( &file );
...
void ExportCSV::doExport()
{
    ...
    if ( !file.open( QIODevice::WriteOnly ) )
    {
        QMessageBox::critical (0, tr("Critical Export Error"), tr("Could not export as CSV to %1").arg(filePath));
        mainWindow->statusMessage(QString(QObject::tr("Export failed.")));
        return;
    }
    QTextStream ts( &file );
...
QString ExportHTML::getBranchText(BranchItem *current)
{
    if (current)
    {
        ...
        // Include image (in branch in HTML-Export: FIXME-3 experimental)
...
void ExportOrgMode::doExport() 
{
    ...
    if ( !file.open( QIODevice::WriteOnly ) )
    {
        QMessageBox::critical (0, tr("Critical Export Error"), tr("Could not export as OrgMode to %1").arg(filePath));
        mainWindow->statusMessage(QString(QObject::tr("Export failed.")));
        return;
    }
    QTextStream ts( &file );
